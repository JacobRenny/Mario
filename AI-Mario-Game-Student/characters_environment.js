...
function game() {
  ...
  if (gameConfig.status === "start") {
    ...
    text(
      ...
      gameConfig.screenX / 2,
      gameConfig.screenY / 2
    );
    textSize(40);
  ...
  if (gameConfig.status === "gameover") {
    ...
    text(
      ...
      gameConfig.screenX / 2,
      gameConfig.screenY / 2 + 135
    );
    textSize(40);
    text(
      round(gameConfig.scores),
      gameConfig.screenX / 2,
      gameConfig.screenY / 2 - 35
    );
...
function positionOfCharacter(character) {
  ...
  enemyMushrooms.forEach(function (element) {
    ...
    if (
      (element.touching.left || element.touching.right) &&
      character.live &&
      character.killing === 0
    )
      die(mario);
...
function dontGetOutOfScreen(character) {
  ...
  if (
    character.position.y > gameConfig.screenY &&
    character.live &&
    character == mario
  ) {
    die(mario);
...
function stateOfEnemy(enemy) {
  if (enemy.live == false || enemy.position.y > gameConfig.screenY + 50) {
    enemy.position.x = random(
      gameConfig.screenX * 1.5,
      2 * gameConfig.screenX + 50
    );
    enemy.position.y = random(
      gameConfig.screenY * 0.35,
      gameConfig.screenY * 0.75
    );
    enemy.live = true;
  }
}
  ...
  group.forEach(function (element) {
    if (element.position.x > -50) {
      ...
      if (group === pipes || group === mountains) {
        element.position.x =
          random(50, gameConfig.screenX) + gameConfig.screenX;
      }
      ...
      if (group === coins) {
        ...
        element.position.y = random(
          gameConfig.screenY * 0.2,
          gameConfig.screenY * 0.8
        );
      }
    }
  });
...
function scores(character) {
  ...
  if (character.live && gameConfig.status === "play")
    gameConfig.timeScores += 0.05;
  ...
  if (mario.live == false && mario.liveNumber != 0) {
    ...
    arc(
      gameConfig.screenX / 2,
      gameConfig.screenY / 2 - 30,
      150,
      150,
      PI + HALF_PI,
      PI + HALF_PI + TWO_PI * ratio
    );
    fill(255, 255, 255);
    noStroke();
    textAlign(CENTER);
...
function outline(obj) {
  rect(
    leftSide(obj),
    upSide(obj),
    rightSide(obj) - leftSide(obj),
    downSide(obj) - upSide(obj)
  );
}
